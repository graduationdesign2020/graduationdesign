...
def send_message(teacher_id, title, content):
    try:
        ...
        coll_message.insert_one({"_id": result[0], "content": content, "title": title, "type": 2, "keys": []})
...
def send_reply_message(teacher_id, title, content, keys):
    try:
        ...
        cursor.execute("""select last_insert_id() from teachermessage""")
        result = cursor.fetchone()
        coll_message.insert_one({"_id": result[0], "content": content, "title": title, "type": 2, "keys": keys})
