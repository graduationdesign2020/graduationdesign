    ...
    def process_item(self, item, spider):
        ...
        if item["item_type"] == 'student':
            ...
            if self.cursor.execute(query_sql,
                                   (base64.encodestring(item['id']).strip(), '学生', item['major'], item['department'])) == 0:
                query_sql = """select * from student where id=%s"""
                if self.cursor.execute(query_sql, (base64.encodestring(item['id'])).strip()) == 0:
                    print('insert student')
                    insert_sql = """insert into student(id, name, major, department) VALUES (%s,%s,%s,%s)"""
                    try:
                        self.cursor.execute(insert_sql,
                                            (base64.encodestring(item['id'].strip()), '学生', item['major'], item['department']))
                        self.connect.commit()
                    except:
                    ...
                    try:
                        self.cursor.execute(update_sql,
                                            ('学生', item['major'], item['department'], base64.encodestring(item['id']).strip()))
                        self.connect.commit()
                    except:
                        self.connect.rollback()
            else:
        ...
        if item["item_type"] == 'grade':
            ...
            if self.cursor.execute(query_sql, (base64.encodestring(item['id']).strip(), item['teacher_grade'],
                ...
                if self.cursor.execute(query_sql, (base64.encodestring(item['id']).strip())) == 0:
                    ...
                    try:
                        self.cursor.execute(insert_sql,
                                            (base64.encodestring(item['id']).strip(), item['teacher_grade'],
                    ...
                    try:
                        self.cursor.execute(update_sql,
                                            (item['teacher_grade'], item['review_grade'], item['defense_grade'],
                                             item['total_grade'], base64.encodestring(item['id']).strip()))
                        self.connect.commit()
                    except:
                        self.connect.rollback()
            else:
        ...
        if item["item_type"] == 'state':
            ...
            if self.cursor.execute(query_sql,
                                   (base64.encodestring(item['project_id']).strip(), '项目名称', item['teacher_id'])) == 0:
                query_sql = """select * from project where id=%s"""
                if self.cursor.execute(query_sql, (base64.encodestring(item['project_id']).strip())) == 0:
                    print('insert project')
                    insert_sql = """insert into project(id, project_name, teacher_id) VALUES (%s,%s,%s)"""
                    try:
                        self.cursor.execute(insert_sql,
                                            (base64.encodestring(item['project_id']).strip(), '项目名称', item['teacher_id']))
                        self.connect.commit()
                    except:
                        self.connect.rollback()
                else:
                    ...
                    try:
                        self.cursor.execute(update_sql,
                                           ('项目名称', item['teacher_id'], base64.encodestring(item['project_id']).strip()))
                        self.connect.commit()
                    except:
                        self.connect.rollback()
            else:
            ...
            if self.cursor.execute(query_sql,
                                   (base64.encodestring(item['project_id']).strip(), item['state'], item['submit'])) == 0:
                query_sql = """select * from state where project_id=%s and state=%s"""
                if self.cursor.execute(query_sql, (base64.encodestring(item['project_id']).strip(), item['state'])) == 0:
                    print('insert state')
                    insert_sql = """insert into state(project_id, state, submit) VALUES (%s,%s,%s)"""
                    try:
                        self.cursor.execute(insert_sql,
                                            (base64.encodestring(item['project_id']).strip(), item['state'], item['submit']))
                        self.connect.commit()
                    except:
                        self.connect.rollback()
                else:
                    ...
                    try:
                        self.cursor.execute(update_sql,
                                            (item['submit'], base64.encodestring(item['project_id']).strip(), item['state']))
                        self.connect.commit()
                    except:
                        self.connect.rollback()
            else:
