import scrapy
import re

from bysjstate.items import BysjstateItem

class StateSpider(scrapy.Spider):
    name = 'state'
    allowed_domains = ['bysj.jwc.sjtu.edu.cn']

    def start_requests(self):
        url = "http://bysj.jwc.sjtu.edu.cn/index.aspx"
        post_data = {
            "__VIEWSTATE": "",
            "__VIEWSTATEGENERATOR": "90059987",
            "__EVENTVALIDATION": "/wEWCQLWrY7BCgLr/4HeAgLmmdGVDAL+jNCfDwK4ovrCAQKdw8XHCAL0uc/cCgLW4JOwDwL4oeqeCcVu583ur2LZLPjfOqwHBkUUnfNV",
            "UserId": "10367",
            "Pwd": "10367",
            "LoginButton.x": "13",
            "LoginButton.y": "6"
        }
        yield scrapy.FormRequest(url=url,
                                 formdata=post_data,
                                 callback=self.character,
                                 method='POST'
                                 )

    def character(self, response):
        url = "http://bysj.jwc.sjtu.edu.cn/LoginRole.aspx?sid=Jk7UDmg%2fcSOg6wXocnYmog....&screen=C3R1RXfU%2fLYcx2ms2xw7Cg...."
        post_data = {
            "__VIEWSTATE": "/wEPDwUKMTk2MTAzODE4Mw9kFgICAw9kFgICAw8PFgIeB1Zpc2libGVnZGQYAQUeX19Db250cm9sc1JlcXVpcmVQb3N0QmFja0tleV9fFgIFDEltYWdlQnV0dG9uMwUMSW1hZ2VCdXR0b240/IfDl20NUC4dCF++M3glQRSC1r8=",
            "__VIEWSTATEGENERATOR": "6CA0729C",
            "__EVENTVALIDATION": "/wEWAwL7oojgAgLSwsGJCgLSwv2aBF/cHhCiQ2K1WnQPUn14ruv9Znzy",
            "ImageButton4.x": "43",
            "ImageButton4.y": "22"
        }
        yield scrapy.FormRequest(url=url,
                                 formdata=post_data,
                                 callback=self.parse_list,
                                 method='POST'
                                 )

    def parse_list(self, response):
        url_list = ['http://bysj.jwc.sjtu.edu.cn/Admin/TotalcxzyRWS.aspx?type=4&sid=Jk7UDmg%2fcSOg6wXocnYmog....&screen=Qd2HRMrs8oJGtA82doE7Jg....',
                    'http://bysj.jwc.sjtu.edu.cn/Admin/kaiticx.aspx?type=5&sid=Jk7UDmg%2fcSOg6wXocnYmog....&screen=Qd2HRMrs8oJGtA82doE7Jg....',
                    'http://bysj.jwc.sjtu.edu.cn/Admin/SelZQJCList.aspx?sid=Jk7UDmg%2fcSOg6wXocnYmog....&screen=Qd2HRMrs8oJGtA82doE7Jg....']
        i = -1
        for url in url_list:
            i = i + 1
            yield scrapy.Request(url=url,
                                 callback=self.parse_form,
                                 meta={'state': str(i)},
                                 method='GET'
                                 )

    def parse_form(self, response):
        state = response.meta['state']

        td_list = response.xpath('//*[@id="gridview"]/tr[2]/td').extract()
        for i in range(3, len(td_list)+1):
            url = response.xpath('//*[@id="gridview"]/tr[2]/td['+str(i)+']/font').css('a::attr(href)').extract()[0]
            num = response.xpath('//*[@id="gridview"]/tr[2]/td['+str(i)+']/font').css('a::text').extract()[0].strip()
            if int(num) == 0:
                continue
            else:
                if int(state) == 0:
                    yield scrapy.Request(url='http://bysj.jwc.sjtu.edu.cn/Admin/' + url,
                                         callback=self.parse_first,
                                         meta={'state': state, 'submit': str(i - 2),
                                               'url': 'http://bysj.jwc.sjtu.edu.cn/Admin/' + url},
                                         method='GET',
                                         dont_filter=True
                                         )
                else:
                    yield scrapy.Request(url='http://bysj.jwc.sjtu.edu.cn' + url[2:],
                                         callback=self.parse_first,
                                         meta={'state': state, 'submit': str(i - 2),
                                               'url': 'http://bysj.jwc.sjtu.edu.cn' + url[2:]},
                                         method='GET',
                                         dont_filter=True
                                         )

    def parse_first(self, response):
        state = response.meta['state']
        submit = response.meta['submit']
        url = response.meta['url']

        n = len(response.xpath('//*[@id="gridview"]/tr').extract())

        for i in range(2, n + 1):
            item = BysjstateItem()
            numstring = response.xpath('//*[@id="gridview"]/tr[' + str(i) + ']/td[4]/font/a/text()').extract()[
                1].strip()
            item['id'] = "".join(re.findall(r'[(](.*?)[)]', numstring))
            item['state'] = state
            item['submit'] = submit
            yield item

        string = response.xpath('//*[@id="LblPageCount"]/text()').extract()[0].strip()
        num = int("".join(re.findall('\d+', string)))
        for i in range(1, num):
            yield scrapy.FormRequest.from_response(
                response=response,
                formdata={'__EVENTTARGET': 'btnNext', 'ddlCurrentPage': str(i)},
                callback=self.parse_next,
                meta={'state': state, 'submit': submit}
            )

    def parse_next(self, response):
        n = len(response.xpath('//*[@id="gridview"]/tr').extract())
        state = response.meta['state']
        submit = response.meta['submit']

        for i in range(2, n + 1):
            item = BysjstateItem()
            numstring = response.xpath('//*[@id="gridview"]/tr[' + str(i) + ']/td[4]/font/a/text()').extract()[
                1].strip()
            item['id'] = "".join(re.findall(r'[(](.*?)[)]', numstring))
            item['state'] = state
            item['submit'] = submit
            yield item

